...
function createReplyElements(replyData) {
    ...
    for (const reply of replyData) {
        ...
        user_reply_link_href_url = base_url + "users/view-profile/" + reply.reply_user_id
        ...
        <span class="host-image-username" id="host-image-username">
          <div class="host-user-image-container"> 
            <a href="${user_reply_link_href_url}" class="reply-personality">
              ...
              <h5 class="color1">${replyUsername}</h5>
            </a>
          </div>
          <!--<p class="replied color1">replied..</p>-->
          <p class="green">${formatTimesince(reply.created)}</p>
    ...
    // } 
        ...
        if (username !== data.chatter_username) {
            ...
            if (createReplyteParagraph.innerHTML === '') {
                ...
                create_new_question.innerHTML = 'Reply ' + data.chatter_firstname + ': ';
                messageToReply.appendChild(create_new_question);
            }
            createReplyteParagraph.innerHTML = data.message_body.slice(0, 50) + '...';
...
function createMessage(data, command) {
    ...
    hostImageLink.href = base_url + "users/view-profile/" + data.user_id; // Assuming you have the user profile URL in the data
    ...
    // var aws_host_url = 'http://127.0.0.1:8000/images/';davedave
